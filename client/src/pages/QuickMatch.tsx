...
export default function QuickMatch() {
  ...
  const handleSearch = useCallback((cfg: PlayerConfig) => {
    setPlayerConfig(cfg);
    setError('');
    getSocket().emit('join_queue', { name: cfg.player1Name });
    setPhase('searching');
  }, []);
        ...
        {(phase === 'playing' || phase === 'ended') && (
          ...
          >
            <div className="flex items-center justify-between text-sm">
              <span className={`font-semibold px-3 py-1 rounded-full ${isMyTurn ? 'bg-primary/30 text-primary' : 'bg-white/10 text-white/40'}`}>
                {myName} ({mySymbol}) {isMyTurn && phase === 'playing' ? '— sua vez!' : ''}
              </span>
              <span className="text-white/30 text-xs">vs {opponentName || 'Oponente'}</span>
            </div>
